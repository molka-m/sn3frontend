    ...
    const filterNavItemsByRole = (items: NavItem[], role: string): NavItem[] => {
      return items
        ...
        .map(item => {
          const newItem = {...item};
          if (newItem.children) {
            newItem.children = filterNavItemsByRole(newItem.children, role);
          }
          return newItem;
        });
    };

    this.navItems = filterNavItemsByRole(this.navItems, userRole);
  }
}
